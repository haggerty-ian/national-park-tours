...
def monitor_tour(request: HttpRequest):
    ...
    if (request.method == 'POST'):
        ...
        return HttpResponse(template.render({'facilities': facilities,
                                             'selected_facility': facility,
                                             'tours': tours,
                                             'selected_tour': tour}, request))
...
def submit_monitor(request: HttpRequest):
    ...
    if not facility or not tour or not email:
        ...
        try:
            ...
            monitor_window = MonitorWindow(email=user_email,
...
def view_monitors(request: HttpRequest, id: int):
    ...
    if len(user_emails) != 0:
        ...
        summaries = \
            list(map(
                lambda monitor: f"""{monitor.tour.facility.name} -
                                    {monitor.tour.name}:
                                    {monitor.start_date} -
                                    {monitor.end_date}""",
                relevant_monitors))
    else:
        summaries = [f"no user email found for id {id}"]
